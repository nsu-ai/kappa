...
Built with Rust + [PyO3](https://pyo3.rs). **PyPI:** `kf-sdk` · **import:** `import kappa_apk` (distribution name kept for continuity with earlier releases).
...
# Kappa — ϰ-framework for dataset and model management, version 3.0.5
...
with KappaApkClient(base_url, "user@example.com", "secret") as client:
    ...
    print(info.data_path)   # OS cache: kappa-framework/datasets/...
